...
Note the two middle lines: the first is the work, the second is the
transaction boundary. **In the failure case you see the first without the
second** — which is exactly what a rollback looks like from the outside.

The id matters under load. Fire concurrent requests and the lines interleave:
...
report the exact id to look up. Send that header **in** and it is used instead
of a generated one, which is how an id follows a request across services.

`--no-access-log` is set: the middleware already records method, path, status
and duration, with the id attached — uvicorn's own access line runs outside
the middleware and could not carry it.

`/health` and `/docs` are excluded from bracketing so probes don't drown out
real traffic.
